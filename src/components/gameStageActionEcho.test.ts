...
function findStatusPill(node: unknown, label: string): { className?: string; children?: unknown; value?: string | null } | null {
  if (Array.isArray(node)) {
    for (const child of node) {
      const match: { className?: string; children?: unknown; value?: string | null } | null = findStatusPill(child, label);
      if (match) {
        return match;
      }
    }
...
async function createInteractionHarness(options: InteractionHarnessOptions = {}) {
  ...
  const applyMiss = vi.fn<(arg: unknown) => ActionResult>(() => ({ kind: "miss" }));
  const queuedPressResults = [...(options.pressResults ?? [])];
  const applyAccuratePress = vi.fn<(arg: unknown, id: string) => ActionResult>(() => {
