...
- Open debt (tracked 2026-08-01): `docs/plans/worm-ranch-plan.md` is referenced by AGENTS.md but missing from disk — restore it or update the AGENTS.md pointer so the next agent is not misled. The neon-token CSS aliases (`--bg`, `--accent`, `--danger`, etc.) are kept "for backward compat during transition" and should be deleted on a set deadline. The fonts/backdrop assets (`public/fonts`, `public/art/Gameplay backdrops/output.mp4`) are untracked — commit or gitignore deliberately before Vercel deploy.
